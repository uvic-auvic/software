...
/*
    can_manager will publish all messages to specific topics on the n("peripherals") namespace.
        -   That means to access them, the node must use a nodehandle of n("peripherals").
...
void parse_json(std::vector<Device_Property> & json_properties, std::string json_file_location) {
    ptree pt;
    //ERROR HERE: kills the node for some reason
...
int main(int argc, char ** argv) {
    ...
    // can_manager begins
    Can_Manager can_manager(json_properties, &n, &nh);
    // TODO: Check what devices are on the bus
    //can_manager.setup(&nh);
